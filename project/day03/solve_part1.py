...
class Grid:
    ...
    def has_adjacent_symbol(self, start_pos: int, end_pos: int):
        ...
        right_pos = end_pos if end_pos % self.width < (self.width - 1) else end_pos -1
        ...
        if right_pos % self.width > 0:  # right_pos is never in 0'th col
...
def main(source_file):
    ...
    for match in grid.finditer(r'\b(\d+)\b'):
        if grid.has_adjacent_symbol(match.start(), match.end()):
            logger.debug('"%s" [%d:%d] has a neighbor symbol', match.group(),match.start(),match.end())
            answer += int(match.group())
        else:
            logger.debug('"%s" [%d:%d] DOES NOT HAVE a neighbor symbol', match.group(),match.start(),match.end())
    return answer
